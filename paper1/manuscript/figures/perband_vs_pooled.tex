...
\node[titlestyle, anchor=south] at (9.75,5.05) {(c) \texttt{pooled\_graph}: one shared graph};
...
\node[font=\sffamily\tiny\itshape, black!55, text width=5.8cm] at ($(Rstart)+(2.7,-0.85)$) {raw band \emph{values} pooled \emph{before} graph construction -- one adjacency $\mathbf{A}_{\text{shared}}$ for all $K$ bands};
