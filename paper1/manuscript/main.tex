...
Figure~\ref{fig:perband-vs-pooled} shows the architectural difference RQ1 asks about: variant (a), the full model of Figure~\ref{fig:architecture}, learns $K=5$ independent adjacency matrices and runs $K$ separate GAT+LSTM branches before fusing them; variant (c) pools the band representations first and learns a single shared adjacency over that pooled representation. The remaining four variants (b, d, and the two capacity settings of c) are described below.
