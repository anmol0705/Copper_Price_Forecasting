...
\begin{enumerate}
    ...
    \item \textbf{Aluminum (\texttt{ALI=F}) shows signs of thin trading, and its close-price series should be read with that caveat.} Direct inspection of the underlying Yahoo Finance data for this ticker shows 63\% zero-volume days and a high-strictly-greater-than-low day on only 15.4\% of days across the full 2010--2025 sample, both signatures of a thinly traded or partially stale-quoted instrument rather than one with continuous daily price discovery; on a substantial majority of days, \texttt{ALI=F}'s reported close plausibly reflects some carry-forward of a prior stale quote rather than a fresh, independently discovered price. We did not drop or replace \texttt{ALI=F} on this basis: its correlation with copper returns ($r=0.41$) is a real, literature-consistent signal independently verified in a sibling project's econometric analysis, not an artifact of the illiquidity itself, and no equally liquid Yahoo Finance substitute for an aluminum-specific series was identified. Readers should nonetheless treat the aluminum node's daily-frequency price movements, as distinct from its correlation with copper at lower frequency, with this liquidity caveat in mind rather than at face value.
