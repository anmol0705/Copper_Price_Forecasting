...
Six variants isolate the contribution of each architectural component, all trained at the untuned default configuration (Section~\ref{sec:impl}, Section~\ref{sec:complexity})---HPO (Section~\ref{sec:hpo}) is applied only to the main-table VMD-MFGNN, never to any ablation variant. The ablation results below are reported as point estimates per horizon; a Diebold-Mariano test on this paper's single central ablation comparison (\texttt{full\_model} vs.\ \texttt{pooled\_graph\_matched\_params}) is reported alongside Table~\ref{tab:ablation} in Section~\ref{sec:ablation-results}, and the remaining ablation pairs are left untested, for the reasons given in Section~\ref{sec:experiments}. A separate five-seed repetition of the two variants that carry RQ1, run later at the tuned configuration and therefore not directly comparable to this table, supplies a seed-level paired test and is reported in Section~\ref{sec:robustness}.
...
Table~\ref{tab:dm} reports the Diebold-Mariano statistic and $p$-value for VMD-MFGNN's forecast errors against each of the six baselines, per horizon (Section~\ref{sec:experiments}). Recall the sign convention (Section~\ref{sec:experiments}): the DM statistic is computed as $e_{\text{VMD-MFGNN}}^2 - e_{\text{baseline}}^2$, so a \emph{negative} DM statistic means VMD-MFGNN had the lower squared error (VMD-MFGNN better), and a \emph{positive} DM statistic means the baseline had the lower squared error (baseline better); boldface marks $p<0.05$. This 24-test battery covers only the baseline comparison; the same test is separately applied to this paper's central ablation comparison in Section~\ref{sec:ablation-results}, but not to the remaining ablation pairs (Section~\ref{sec:experiments} explains the scope decision). All 24 test entries here are finite (no degenerate $\mathrm{var}_d \le 0$ cases were observed at full data scale).
...
\textbf{This table does not support the paper's central architectural claim, and we report that plainly, stated precisely rather than more strongly than the numbers allow.} This is the corrected re-run: every variant now uses the \texttt{min\_epochs}=10 checkpoint-selection floor (Section~\ref{sec:impl}), and per-sample predictions were archived for the first time, so both the numbers and the sign-concentration flags below are independently verified rather than inferred. The precise claim is: row (a), the per-band full model, attains the \emph{best-of-six} value in exactly one of twelve RMSE/MAE/DA columns --- DA at $h=22$ (48.2\%), a clean win not subject to any sign-concentration or base-rate-identity flag. Against \texttt{pooled\_graph\_matched\_params} (397{,}586 vs.\ 393{,}476 params, $+$1.05\%, the comparison most directly targeting RQ1 with capacity approximately controlled), the full model now loses RMSE and MAE at \emph{all four} horizons ($h=1,5,10,22$), reversing what an earlier, since-corrected run of this same comparison reported; a Diebold-Mariano test on this exact pair, made possible for the first time by this run's newly-archived per-sample predictions, finds none of these four RMSE differences statistically significant (DM$=+1.54,+0.24,+0.33,+1.37$ at $h=1,5,10,22$; $p=0.124,0.813,0.743,0.172$ respectively; none significant even before any multiple-testing correction is applied). Directional accuracy tells the opposite story on this same pairing: the full model beats \texttt{pooled\_graph\_matched\_params} on DA at \emph{three of four} horizons ($h=5$: 49.7\% vs.\ 48.1\%; $h=10$: 46.5\% vs.\ 44.6\%; $h=22$: 48.2\% vs.\ 47.2\%), losing only at $h=1$ (48.6\% vs.\ 50.6\%); none of these four DA-column cells for either variant carry a base-rate-\emph{identity} flag (Section~\ref{sec:constantforecast}), but we do not read this DA edge fully at face value: \texttt{pooled\_graph\_matched\_params} itself clears the softer $\geq90\%$ sign-concentration bar at exactly the three horizons where the full model beats it on DA ($h=5$: 94.4\%; $h=10$: 99.4\%; $h=22$: 94.7\% majority-sign, all negative), so at least part of this DA gap may reflect \texttt{pooled\_graph\_matched\_params}'s own near-constant output rather than a genuine directional advantage for the full model. We flag this explicitly rather than let the three-of-four framing stand uncontested, and note that this paper's significance-testing machinery (Section~\ref{sec:experiments}) is built around the Diebold-Mariano test's squared-error loss differential and was not extended to a paired test for DA (a binary, non-continuous statistic), so this specific DA comparison, unlike the RMSE one above it, is not backed by a formal significance test either way. Taken together on this single, capacity-matched, most-interpretable pairing: the per-band full model shows a real but statistically unconfirmed RMSE/MAE disadvantage and a directionally consistent, also-unconfirmed, and partly sign-concentration-confounded DA advantage, which is a materially different and more nuanced picture than either variant cleanly beating the other. This is the honest summary we take forward into Section~\ref{sec:discussion}: the per-band mechanism does not demonstrate the clear advantage it was designed to produce, but the earlier claim that it \emph{never} wins any comparison did not survive a corrected re-run and is retracted here in favor of this more qualified one. We applied this paper's sign-concentration and base-rate-identity check (Section~\ref{sec:constantforecast}) to every cell of this table directly, using the per-sample predictions this run is the first to archive: eight cells are \emph{confirmed} base-rate artifacts (majority-sign $\geq90\%$ \emph{and} an exact class-frequency match, both independently verified, not inferred from the DA value alone) --- all eight, in fact, are 100.0\% sign-concentrated (every single prediction shares the same sign), the strongest form this artifact can take: \texttt{no\_vmd\_raw\_price\_matched\_dim} at $h=10$ (DA 44.2\%, 100.0\% negative-sign) and $h=22$ (44.7\%, 100.0\%), \texttt{no\_vmd\_raw\_price\_matched\_params} at all four horizons (48.4\%, 47.0\%, 44.2\%, 44.7\% DA, each 100.0\% negative-sign), and \texttt{pooled\_graph\_matched\_dim} at $h=5$ (47.0\%, 100.0\% negative-sign) and $h=22$ (44.7\%, 100.0\% negative-sign); these are marked $^{\ddagger}$ in Table~\ref{tab:ablation}. Notably, three of the six variants (\texttt{no\_vmd\_raw\_price\_matched\_dim}, \texttt{no\_vmd\_raw\_price\_matched\_params}, and \texttt{pooled\_graph\_matched\_dim}) land on the exact same confirmed-artifact value at $h=22$ (44.715\%, the test period's down-day frequency to three decimal places) simultaneously --- half of this table's variants are confirmed near-constant, always-down-day forecasters at the longest horizon, which we read as a further symptom of how little real directional signal this untuned configuration extracts at $h=22$ specifically, independent of architecture. A further eight cells clear the softer $\geq90\%$ sign-concentration bar without an exact class-frequency match (\texttt{no\_vmd\_raw\_price\_matched\_dim} at $h=5$, 93.2\%; \texttt{pooled\_graph\_matched\_dim} at $h=1$ and $h=10$, 97.0\% and 99.1\%; \texttt{full\_model} at $h=10$, 90.1\%; \texttt{pooled\_graph\_matched\_params} at $h=5$, $h=10$, and $h=22$, 94.4\%, 99.4\%, and 94.7\% respectively; and \texttt{correlation\_graph} at $h=1$, 96.5\%) and are not marked in the table, consistent with this paper's convention of marking only confirmed artifacts, but are worth flagging here since two of them (\texttt{full\_model} and \texttt{pooled\_graph\_matched\_params} at $h=10$, and \texttt{pooled\_graph\_matched\_params} more broadly at $h=5,10,22$) sit inside the very head-to-head comparison discussed above and are already addressed there.
...
Section~\ref{sec:collapse}'s diagnosis identified two candidate mechanical causes of the adjacency collapse: coupled $L_2$ weight decay pulling every embedding parameter toward the origin, and the absence of any normalization on the bilinear logits before softmax, which lets embedding magnitude itself (rather than only direction) determine how peaked or flat the resulting adjacency is. A diagnosis that stops at ``this is probably why'' is weaker than one that tests the proposed mechanism directly, so we implemented and trained with two targeted fixes together: (1) $L_2$-normalizing $\mathbf{E}^{(1)}_k$ and $\mathbf{E}^{(2)}_k$ to the unit sphere immediately before the bilinear product, so the softmax input's scale can no longer collapse toward the origin regardless of what happens to the embeddings' raw magnitude; and (2) excluding the graph-embedding parameters from Adam's coupled weight decay entirely (a dedicated zero-decay parameter group), removing the constant, sign-consistent pull toward zero identified as the proximate driver of the collapse. Both fixes were applied together, not separately, since the diagnosis in Section~\ref{sec:collapse} treats them as jointly responsible for the same failure mode. We retrained both \texttt{full\_model\_graphfix} (per-band) and \texttt{pooled\_graph\_graphfix} (pooled) with both fixes enabled; an earlier version of this run additionally suffered from the same checkpoint-selection bug disclosed in Section~\ref{sec:impl} (\texttt{full\_model\_graphfix} in that run early-stopped at an effectively untrained, near-epoch-0 checkpoint), so the numbers reported below are from a corrected re-run with the same \texttt{min\_epochs}=10 floor used in Table~\ref{tab:ablation}, in which \texttt{full\_model\_graphfix} trained to a genuine 57 epochs and \texttt{pooled\_graph\_graphfix} to 27.

\textbf{The fix mechanically works.} Direct inspection of the trained checkpoints confirms the norm-collapse mechanism is resolved: embedding RMS magnitude is preserved at 0.96--1.01$\times$ its Xavier-initialization value across bands and models, in sharp contrast to the $\approx$3\% of initialization observed in the unfixed checkpoints of Section~\ref{sec:collapse} and Section~\ref{sec:confound-tuned}. We independently re-verified this from the raw checkpoint state dictionaries rather than only citing the fix-experiment's own log: the pooled model's \texttt{graph\_constructor.emb1/emb2} weight RMS (0.302 and 0.279) and the per-band model's five \texttt{graph\_constructors.\{0..4\}.emb1/emb2} weight RMS values (0.27--0.30) both sit close to the 0.2885 Xavier-initialization value reported in Section~\ref{sec:collapse}, not the collapsed $\sim$0.009 value observed there. Whatever was driving the embeddings to zero in every earlier checkpoint no longer does so once weight decay is removed from these parameters and the bilinear product is computed on unit-normalized vectors.

\textbf{The fixed mechanism still learns nothing --- and a second, independent reason why is now directly measured.} Mechanically preserving embedding norm is not the same as learning a functional graph, and it does not produce one here. The corrected embeddings moved only 0.6--2.0\% in L2 norm away from their random initialization over the full training run, and retained cosine similarity of 0.9998--1.0000 with their own initialization---meaning the small amount of movement that did occur was almost entirely a rescaling within the same random-init direction, not a rotation toward any task-informed direction. The resulting adjacency matrices correlate with the adjacency computed from the \emph{untrained}, randomly initialized embeddings at 0.994--0.999 per band. This is not the uniform-collapse signature of Section~\ref{sec:collapse} reappearing under a different name: measured on the same dense, pre-sparsification basis as Section~\ref{sec:collapse}'s $\approx2.8\times10^{-4}$ figure, the fixed adjacency's softmax standard deviation is 0.013--0.020 per band (\texttt{full\_model\_graphfix}) and 0.017 (\texttt{pooled\_graph\_graphfix}) --- roughly 50--70$\times$ larger, about 1.7--1.9 orders of magnitude --- and only 0--15.6\% of edges land within tolerance of the collapsed model's near-universal $1/N$ value, so the fixed graph is not flat in the way the unfixed one is; it is simply the same graph a random Xavier initialization would already produce, never having moved meaningfully away from it. Put plainly: once norm collapse is no longer available as an explanation, the ``fixed'' per-band graph is statistically indistinguishable from a graph that was never trained at all. We took this one step further than inferring a weak-gradient explanation from the embeddings' final resting position: we directly measured the gradient reaching $\mathbf{E}^{(1)}_k,\mathbf{E}^{(2)}_k$ at two points --- a freshly initialized model with the fix enabled, and the genuinely-trained 57/27-epoch checkpoints above --- using the same gradient-magnitude diagnostic applied throughout Section~\ref{sec:robustness}. At initialization, the fix does not structurally block gradient flow: the aggregate embedding-gradient norm is small but clearly nonzero ($2.46\times10^{-6}$, averaged over 5 batches, range $1.4$--$3.8\times10^{-6}$) and statistically indistinguishable from the same measurement with the fix disabled ($3.17\times10^{-6}$, range $1.8$--$5.5\times10^{-6}$), against a same-model non-embedding gradient norm of $1.564$ in both cases --- so whatever prevents learning here is not a severed autograd path introduced by unit-normalizing the embeddings. Yet at the genuinely-trained checkpoints, the identical measurement returns \emph{exactly} $0.0$ across every one of 5 sampled batches, for both \texttt{full\_model\_graphfix} and \texttt{pooled\_graph\_graphfix}, against a healthy non-embedding gradient norm of $0.155$--$0.168$ in the same forward-backward passes. This is a more precise finding than ``the task gradient is weak'': the gradient reaching these specific parameters is small but real at step zero, and decays to a numerically exact zero somewhere over the course of training, \emph{independent of weight decay}, since weight decay has been excluded from these parameters entirely in this experiment. Removing weight decay's pull did not substitute a learning signal for it; it unmasked a second, independent reason no learning occurs, one that operates on the gradient itself rather than on the embedding norm, and that this paper has not further decomposed --- we did not trace which downstream computation (the top-$k$ sparsification's discrete edge selection stabilizing, or a saturation region of the cosine-similarity bilinear form once training pushes an embedding row's pre-ReLU logits fully negative or fully positive) is responsible, and flag this as a genuine open question rather than a diagnosed mechanism.

We read this as strengthening, not weakening, the paper's account of the collapse: we diagnosed a specific mechanism, engineered a fix targeted precisely at that mechanism, confirmed the fix resolves the mechanism it was designed to resolve, and found that the architecture still fails to learn differentiated graph structure for a deeper, independently \emph{measured} (not merely inferred) reason. A negative result that survives its own proposed remedy is more informative than one that was never tested against a remedy at all, and the direct gradient measurement sharpens the claim in Section~\ref{sec:collapse} from ``weight decay probably caused this, given a weak task gradient'' to ``removing weight decay's pull does not fix it, because the task gradient reaching these parameters --- present but small at initialization --- vanishes to exactly zero over training regardless of whether decay is acting on them at all.''
...
While investigating the graph-fix checkpoints' predictions directly, in order to understand \texttt{pooled\_graph\_graphfix}'s directional-accuracy numbers, we found something that changes how several directional-accuracy comparisons elsewhere in this paper should be read. (We report this from the corrected, genuinely-trained 27-epoch re-run of this model, Section~\ref{sec:graphfix}; an earlier, near-untrained version of this same checkpoint showed all four horizons degenerate, which the corrected re-run does not fully reproduce, illustrating incidentally why the underlying checkpoint matters and not only the DA figure it produces.) At $h=1$ and $h=22$, this model's 984 test-set predictions are of a single, constant sign (100.0\% negative at both horizons) and its predicted-return dispersion is a small fraction of the true-return dispersion (predicted-to-true standard deviation ratio 0.012 at $h=1$, 0.010 at $h=22$) --- a model whose forecast never changes sign and barely varies in magnitude is not exercising directional judgment on any individual sample; its ``directional accuracy'' at those two horizons can only ever equal the fraction of test days on which the true return happened to share that one sign. At $h=10$ the same model is majority-sign concentrated (93.5\% negative) but not to the same degree, and at $h=5$ it is not degenerate at all by either criterion (only 62.1\% of predictions share a sign, near the healthy range): the same checkpoint is a genuine sign-concentration artifact at two horizons, borderline at a third, and clean at the fourth, which is itself worth noting since it means degeneracy must be checked per horizon rather than assumed uniform across a model's own output heads.

We verified the $h=1$ and $h=22$ artifacts precisely rather than assuming them: recomputing directional accuracy directly from the saved \texttt{results/graphfix\_minepochs\_fix/predictions/pooled\_graph\_graphfix\_\{1,22\}.npy} prediction and true-value arrays reproduces the reported DA figures (48.374 and 44.715 at $h=1,22$) to three decimal places, and each of these values in turn equals, to the same precision, the test period's unconditional frequency of the sign this model always predicts at that horizon (down-day frequency 48.374\% and 44.715\% respectively, computed under the same strict-negative convention stated in Section~\ref{sec:experiments}: a day with an exactly-zero return is counted in neither the up-day nor the down-day frequency, consistent with the DA convention used throughout this paper). This is not a coincidental resemblance; it is an identity, because a constant-sign forecaster's accuracy is mathematically the base rate of the sign it happens to emit, no more and no less. The same pattern reproduces in \texttt{pooled\_graph\_matched\_params\_tuned} from the confound-check experiment (Section~\ref{sec:confound-tuned}), an entirely separate run unaffected by the graph-fix checkpoint correction above: its four DA values (48.37, 53.05, 55.39, 44.72) match the corresponding test-period class frequencies to the same precision, even though we did not archive that run's raw prediction arrays and can only confirm it via this frequency match rather than by inspecting prediction dispersion directly. Cross-referencing further, this pattern is consistent with what motivated this investigation in the first place: multiple models across the original untuned ablation (Section~\ref{sec:ablation-results}), the tuned confound-check (Section~\ref{sec:confound-tuned}), and the fix experiment above show DA values that coincide with plausible class-frequency figures for their horizon, though \texttt{pooled\_graph\_graphfix} (at $h=1,22$) and \texttt{pooled\_graph\_matched\_params\_tuned} are the cases we can state this about with full confidence, having checked prediction dispersion or exact frequency-matching directly. A binomial test of each model's hit count against the base rate of whichever sign that model always emits (not, in general, the horizon's majority-class frequency, since a constant-sign forecaster can just as easily emit the minority sign, as \texttt{pooled\_graph\_graphfix} does at $h=1$ and $h=22$), run across every model$\times$horizon cell in the fix-experiment run, fails to reject that base-rate null at conventional significance levels in every cell tested---these apparent directional-accuracy figures do not represent a statistically demonstrated departure from what the emitted-sign base rate alone would produce.
...
A model can show a statistically significant DM advantage over another forecaster while still carrying no economic value, and RQ0's naive-baseline result already suggests that is the more likely situation here; we check this directly with a lightweight, unlevered long/short economic sanity check (position sized by predicted sign, non-overlapping-period annualization, no explicit transaction-cost model beyond the reported breakeven threshold) computed post hoc from the saved test-set predictions. Across all seven fitted models and four horizons, annualized Sharpe ratios computed this way are inconsistent in sign across horizons for every model, including VMD-MFGNN (ranging from about $-0.45$ to $+1.14$ across the 28 model/horizon combinations, with every model switching sign at least once across horizons); breakeven round-trip transaction costs implied by these Sharpe ratios are, for over half of the model/horizon combinations, in the single-digit basis points or negative, well inside or below realistic copper-futures round-trip transaction costs, and Pesaran-Timmermann directional-value tests \citep{PesaranTimmermann1992} are not significant at conventional levels for the large majority (21 of 28) of model/horizon combinations. The remaining 7, at raw $\alpha=0.05$, are LSTM at $h=5$ ($p=0.039$) and $h=22$ ($p=0.016$), SimpleMTGNN at $h=22$ ($p<0.0001$), Transformer at $h=10$ ($p=0.013$), VMD-LSTM at $h=22$ ($p=0.015$), and XGBoost at $h=10$ ($p=0.021$) and $h=22$ ($p=0.001$); we name them rather than leave a bare count, and, applying Holm-Bonferroni to this 28-test family separately from the DM families above (Section~\ref{sec:limitations}), only 2 survive: SimpleMTGNN at $h=22$ and XGBoost at $h=22$. Both of XGBoost's raw-significant cells carry a \emph{negative} PT statistic, meaning its directional calls are significantly \emph{worse} than chance at those horizons, not evidence of skill; SimpleMTGNN's $h=22$ result is consistent with, rather than contradicting, this section's overall reading, since Section~\ref{sec:main-results} already reports SimpleMTGNN as the most degenerate model in this study at exactly that horizon. We read this as corroborating RQ0 rather than adding a new finding: consistent with no model beating a trivial zero-forecast baseline on MAE, no model in this study demonstrates directional forecasting value robust enough to survive realistic trading frictions, and the occasional positive Sharpe ratio at a single horizon for a single model should be read as noise around zero rather than as evidence of exploitable skill, given how frequently sign and magnitude both flip across adjacent horizons for the same model. VMD-MFGNN's own $h=1$ cell is the largest single value in this range (annualized Sharpe 1.14, breakeven cost 233.5\,bps) and is worth flagging rather than passing over quietly: even so, its underlying Pesaran-Timmermann directional-value test is not significant, raw or Holm-corrected ($p=0.090$), and its DA edge over the majority-class null is only 1.5 percentage points (Section~\ref{sec:main-results}), so we read this single cell as within the noise this section otherwise describes rather than as a standout economic result.
...
\begin{enumerate}
    \item \textbf{Zinc and nickel dropped from scope entirely.} Section~\ref{sec:data} describes why (both proxy tickers confirmed dead/delisted with no viable substitute), reducing this study from an originally planned 10-variable system to the $N=8$ system used throughout. The consequence for interpretation: results and interpretability figures reflect this narrower set of inter-commodity relationships, and any relationship specifically involving zinc or nickel cannot be studied with this dataset.
    ...
    \item \textbf{Daily-refit VMD trades boundary-effect exposure for leakage-safety, and this study does not quantify the trade-off.} Section~\ref{sec:graph} extracts only the final-timestep value of each rolling-window VMD fit as that day's feature, discarding the other 251 points of each 252-day decomposition. This choice is deliberate and disclosed as a leakage-safety measure (Section~\ref{sec:method}), but the endpoint of any windowed spectral decomposition is also its least-constrained point --- anchored by data on only one side rather than both --- a boundary effect well documented for VMD's predecessor, EMD, and reduced but not eliminated in VMD itself. We did not quantify how much this affects the resulting mode values relative to a mid-window (better-constrained but leakage-unsafe) extraction, nor compare against the alternative \citet{Feng2026} use for the same causality-preserving reason: retaining every timestep of each window's decomposition as a feature, rather than only the endpoint, which trades a larger per-sample feature footprint for less information loss. We are not aware of the closest comparable prior work, MBTI-Net \citep{LiLiu2026}, addressing this trade-off either, since its stated ``decomposition performed once and cached'' description does not discuss leakage-safety or boundary effects at all, suggesting it does not face this specific cost because it does not attempt the leakage-safety this study does. Quantifying the boundary-effect cost directly, or adopting a whole-window feature representation, is left to future work.
    ...
    \item \textbf{Aluminum (\texttt{ALI=F}) was removed from the input scope after this study's original results were produced, for a data-integrity reason discovered during a later expansion of the feature set.} \texttt{ALI=F}'s actual Yahoo Finance history begins 2014-05-06, not 2010, despite every other variable in this study's panel beginning 2010-01-04. Because the pipeline drops any trading day with a missing value in any input column, \texttt{ALI=F}'s late start was silently truncating the \emph{entire} panel to a 2014--2025 window---a $\sim$25\% reduction from the $\sim$2010--2025 range and $\sim$4,000-trading-day figure stated elsewhere in this paper---for as long as aluminum remained in scope. \texttt{ALI=F} also shows independent signs of thin trading (63\% zero-volume days, a high-strictly-greater-than-low day on only 15.4\% of days across its available history), consistent with a partially stale-quoted instrument. Rather than shift this study's entire chronological scope, and every date and split boundary reported throughout this paper, to 2014 onward, aluminum was removed from the input variable set entirely; this affects only the expanded-scope results reported alongside this limitation and does not retroactively change the 2010--2025 window already reported for this study's original 8-variable results. No equally liquid, full-history Yahoo Finance aluminum substitute was identified.
    \item \textbf{\texttt{ppiaco} (FRED Producer Price Index: All Commodities) carries a distinct, verified regime-shift vulnerability for two baselines in a later, expanded-scope analysis, disclosed here rather than folded into the main results tables.} As part of a later expansion of the exogenous feature set beyond this paper's original $N=8$ variables (Section~\ref{sec:data}) to include four monthly FRED series merged in on a point-in-time, publication-lag-respecting basis, \texttt{ppiaco} (forward-filled daily) is a genuine, legitimate macroeconomic input, not a data error of the kind corrected in Section~\ref{sec:data}. Direct inspection shows it sits more than 5 standard deviations outside its own training-split (pre-2020) distribution for 100\% of test-period (post-2022) trading days, driven by a real macro regime shift --- the 2022+ inflation surge --- that the training split never observed; critically, this exposure is nearly invisible during model selection, since the same $>5\sigma$ condition holds on only 4.6\% of validation-period (2020--2021) days, meaning early-stopping and checkpoint selection have essentially no visibility into this failure mode before test-time evaluation. Direct inspection of saved prediction arrays from this expanded-scope run confirms this exposure as the driver, not merely a correlate, of a severe test-period performance regression specific to the two baselines that ingest raw exogenous inputs most directly and lack architectural robustness to a shifted input distribution: LSTM ($h=22$ RMSE $0.083\rightarrow0.361$, a $4.3\times$ increase over the same configuration's $N=8$ figure) and Transformer ($h=22$ RMSE $0.105\rightarrow0.187$, $1.8\times$). The mechanism is visible directly in the saved predictions: both models' forecasts collapse toward an almost perfectly constant negative daily drift rate (approximately $-0.011$ to $-0.015$ per day, depending on horizon) that is multiplied by the forecast horizon, explaining why LSTM's RMSE increase scales roughly linearly with $h$ ($0.022\rightarrow0.090\rightarrow0.177\rightarrow0.361$ at $h=1/5/10/22$); Transformer shows the same collapse mechanism less completely, consistent with its smaller regression. Other models tested at this expanded scope are not similarly affected, for reasons that track architecture rather than coincidence: ARIMA (univariate, never ingests \texttt{ppiaco} or any exogenous variable) and VMD-LSTM (strictly univariate per decomposed mode by this project's own design, Section~\ref{sec:baselines}) are structurally unable to be affected; XGBoost is stable because tree splits are invariant to a variable drifting far outside its training range, reacting only to split thresholds; SimpleMTGNN and VMD-MFGNN, both of which use substantial internal LayerNorm re-normalizing activations at every layer, degrade only mildly ($\sim$1.03--1.23$\times$). We keep \texttt{ppiaco} in the expanded input scope rather than dropping or transforming it: we read this as a genuine, citable methodological finding in its own right --- architectural robustness to macroeconomic regime shifts varies systematically across the model families studied here --- rather than a confound to be engineered away, and dropping or transforming it would require invalidating and re-running the entire completed expanded-scope training pipeline, which is out of scope here. This finding is not yet reflected in this paper's main results tables (Table~\ref{tab:main}, Table~\ref{tab:dm}), which report only the original $N=8$ configuration; extending the full comparison to this expanded scope is left to future work.
    ...
    \item \textbf{The pooled-graph ablation is our own construal of MBTI-Net-style pooling, not a reproduction of MBTI-Net itself, and differs from it on one further axis worth naming explicitly.} We did not have access to retrain or inspect MBTI-Net's checkpoints (Section~\ref{sec:related}), so the \texttt{pooled\_graph} variant (Section~\ref{sec:ablation}) is our own architectural instantiation of what "pooling before graph construction" means in this codebase, matched to VMD-MFGNN's other design choices rather than to MBTI-Net's specific ones. The RQ1 comparison this paper reports is therefore evidence about per-band-versus-pooled graph construction as a design axis in general, not a head-to-head benchmark result against MBTI-Net specifically. One specific architectural difference is worth flagging: MBTI-Net's pooled adjacency is \emph{input-conditioned} (constructed from an LSTM-and-temporal-attention context vector that varies per sample), whereas both our per-band and pooled adjacencies are \emph{statically parameterized} (a free embedding table, fixed once training ends, independent of the current input). Our isotropic-collapse diagnosis (Section~\ref{sec:collapse}) is specifically about weight decay shrinking a static embedding table with no counteracting rotation signal; an input-conditioned adjacency has no analogous free table for weight decay to act on and may not be subject to the same collapse mechanism. We do not claim our finding transfers to MBTI-Net's specific graph-construction mechanism, only to statically-parameterized learned adjacencies of this general form, and flag the static-versus-input-conditioned distinction as the boundary of that claim rather than glossing over it. We also note that MBTI-Net's "complexity-aware" decomposition is a fuzzy-entropy-and-log-energy K-means grouping of standard VMD's output modes into three clusters, not a modification to the VMD optimization objective itself, for readers who might otherwise infer a different meaning from the name.
    ...
    \item \textbf{Directional accuracy is not interpretable without a sign-concentration and majority-class-null check, and this paper's own earlier drafts did not apply one.} Section~\ref{sec:constantforecast} documents that at least two models evaluated in this study (\texttt{pooled\_graph\_graphfix}, at $h=1$ and $h=22$ specifically, and \texttt{pooled\_graph\_matched\_params\_tuned}, at all four horizons) emit near-constant, near-zero-variance forecasts whose directional accuracy exactly reproduces the test period's unconditional class frequency rather than reflecting real directional skill, and that this artifact was responsible for an apparent reversal of this paper's central ablation finding that we have since retracted (Section~\ref{sec:confound-tuned}). The corrected ablation re-run (Section~\ref{sec:ablation-results}) is the one exception where this is no longer a gap: per-sample predictions were archived for that run for the first time, so both sign concentration and the exact class-frequency match were independently verified, not inferred from the DA value alone, yielding eight confirmed artifact cells marked $^{\ddagger}$ in Table~\ref{tab:ablation}. Per-sample prediction arrays remain unarchived for the tuned confound-check run (\texttt{pooled\_graph\_matched\_params\_tuned}, Section~\ref{sec:confound-tuned}), so for that variant specifically we still apply only the base-rate-identity half of the check (DA value against the test period's class frequency), which requires just the DA figure and the known class frequencies, not the underlying predictions; that check is what supports the retraction described above. RMSE/MAE comparisons, which are not subject to the same constant-forecast artifact, should still be weighted more heavily than DA comparisons wherever the two are in tension.
    \item \textbf{Multiple testing across 32 significance tests, corrected as three separate families.} This paper runs 32 Diebold-Mariano tests in total, in three families we correct separately rather than pooling: 24 for the baseline comparison (six baselines $\times$ four horizons, Section~\ref{sec:results-dm}), 4 for the naive-baseline comparison (one per horizon, Section~\ref{sec:main-results}), and 4 for this paper's central ablation comparison (\texttt{full\_model} vs.\ \texttt{pooled\_graph\_matched\_params}, one per horizon, Section~\ref{sec:ablation-results}). We apply a Holm-Bonferroni correction at family-wise $\alpha=0.05$ within each family (Section~\ref{sec:experiments}); for the 24-test family this reduces the significant count from 12 to 8 (all favoring VMD-MFGNN) and removes the one significant result favoring a baseline (XGBoost at $h=1$). The ablation family needs no correction in practice: none of its four raw $p$-values ($0.124$--$0.813$) is significant even before any correction is applied, so no Holm step changes that outcome. We disclose the three-family choice explicitly: correcting all 32 as one family would raise the per-test threshold required for significance (roughly $0.0016$ instead of $0.0125$ at the relevant step for the naive-baseline family), and we checked that the $h=1$ naive-baseline result ($p=0.0007$) survives either way, so no reported conclusion depends on this choice, but a reader comparing the three families' corrected counts should know they were not pooled. We report both raw and corrected significance in Table~\ref{tab:dm} rather than only the more favorable raw count.
...
Becerra, M., Jerez, A., Garc\'es, H.O., Demarco, R., 2022. Copper price: a brief analysis of China's impact over its short-term forecasting. Resources Policy 75, 102449.
...
Li, Y., Liu, D., 2026. A hybrid model for copper futures price forecasting utilizing complexity-aware variational mode decomposition and reconstruction and multi-behavior-triggered interaction modeling. Entropy 28(3), 320.
...
\bibitem[Pesaran and Timmermann(1992)]{PesaranTimmermann1992}
