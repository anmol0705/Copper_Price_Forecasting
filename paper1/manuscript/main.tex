...
\begin{table*}[!t]
\centering
\caption{Ablation study results, all four horizons, test split. All variants trained at the untuned default configuration (Section~\ref{sec:impl}); this is a different, smaller VMD-MFGNN instance than the HPO-tuned model in Table~\ref{tab:main}. No significance column (single-split protocol; see Section~\ref{sec:limitations}). Best result per column shown in \textbf{bold}. $^{\ddagger}$Marks DA values that are \emph{candidate} base-rate artifacts: each equals the test period's class frequency for that horizon exactly (Section~\ref{sec:constantforecast}), but --- unlike the confirmed Transformer $h=5$ artifact in Table~\ref{tab:main} --- per-sample predictions were not archived for this untuned ablation run, so sign concentration and dispersion could not be independently checked, and the exact-match criterion alone is not always sufficient (Section~\ref{sec:constantforecast} documents one case, Transformer's $h=22$ cell in Table~\ref{tab:main}, where an exact numeric match turned out to be coincidental rather than a true artifact); two of these ($^{\ddagger}$ cells in the matched-dim $h=5$ and matched-params $h=22$ columns) would otherwise be the bolded best-in-column value and are left unbolded pending that verification.}
\label{tab:ablation}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l cccc cccc cccc}
\toprule
...
(b) No-VMD, matched-dim (88{,}836) & 0.0279 & 0.0400 & 0.0569 & 0.0763 & 0.0231 & 0.0280 & 0.0429 & 0.0575 & 48.4$^{\ddagger}$ & 53.0$^{\ddagger}$ & 46.1 & \textbf{49.4} \\
...
(c) Pooled graph, matched-params (397{,}586) & 0.0225 & 0.0419 & 0.0552 & 0.0762 & 0.0168 & 0.0291 & 0.0401 & \textbf{0.0568} & 50.9 & \textbf{52.9} & 49.8 & 55.3$^{\ddagger}$ \\
