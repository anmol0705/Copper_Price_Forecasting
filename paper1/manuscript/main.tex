...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw=black!70, rounded corners=2pt, fill=black!3, align=center,
              inner sep=3.5pt, minimum height=11mm, text width=39mm},
  io/.style={box, fill=black!10},
  arr/.style={-{Latex[length=2.2mm]}, semithick, black!75},
]
%% ---- per-band chain (Algorithm lines 2--10) ----
\node[box] (gc)
  {\textbf{Per-band graph construction}\\[1pt]
   $\mathbf{E}^{(1)}_k,\mathbf{E}^{(2)}_k\in\mathbb{R}^{N\times d}$\\
   $\mathbf{A}_k=\mathrm{softmax}(\mathrm{ReLU}(\mathbf{E}^{(1)}_k{\mathbf{E}^{(2)}_k}^{\!\top}))$\\
   row-wise top-$k$, $k=4$ of $N=8$};
\node[box, below=6mm of gc] (gat)
  {\textbf{Graph attention}\\[1pt]
   project $\mathbf{u}^{(k)}_t\in\mathbb{R}^{N}\!\to\!\mathbb{R}^{d_h}$;\\
   $H$-head GAT on $\mathbf{A}_k$ at each $t$,\\
   residual $+$ LayerNorm, $\times$ layers};
\node[box, below=6mm of gat] (lstm)
  {\textbf{Temporal LSTM}\\[1pt]
   per variable, over $t=1,\ldots,L$\\
   $\mathbf{z}^{(k)}=\mathrm{LSTM}(\mathbf{h}^{(k)}_1,\ldots,\mathbf{h}^{(k)}_L)$};
\node[box, below=6mm of lstm] (ext)
  {\textbf{Read out target node}\\[1pt]
   copper $=$ node $0$\\
   $\mathbf{z}^{(k)}_{\mathrm{Cu}}\in\mathbb{R}^{d_h}$};
\draw[arr] (gc) -- node[right=1mm, font=\scriptsize] {edge list $+$ weights} (gat);
\draw[arr] (gat) -- node[right=1mm, font=\scriptsize] {$\mathbf{h}^{(k)}_t$} (lstm);
\draw[arr] (lstm) -- (ext);

\begin{scope}[on background layer]
  \node[draw=black!25, dashed, rounded corners=3pt, fill=white,
        fit=(gc)(gat)(lstm)(ext), inner sep=4.5mm, xshift=4mm, yshift=4mm] {};
  \node[draw=black!35, dashed, rounded corners=3pt, fill=white,
        fit=(gc)(gat)(lstm)(ext), inner sep=4.5mm, xshift=2mm, yshift=2mm] {};
  \node[draw=black!60, dashed, rounded corners=3pt, fill=white,
        fit=(gc)(gat)(lstm)(ext), inner sep=4.5mm] (bandbox) {};
\end{scope}
\node[above=6mm of bandbox.north, align=center, font=\small\itshape]
  (bandlabel) {constructed and run independently\\ for each of the $K=5$ frequency bands};

%% ---- input side ----
\node[io, left=13mm of bandbox] (vmd)
  {\textbf{VMD decomposition}\\[1pt]
   per variable, $K=5$ modes;\\
   leakage-safe daily refit,\\
   modes sorted by $\omega_k$\\
   $\mathbf{U}\in\mathbb{R}^{N\times K\times L}$};
\node[io, left=11mm of vmd] (inp)
  {\textbf{Raw input window}\\[1pt]
   $N=8$ commodity / macro series\\
   lookback $L=60$ trading days};

%% ---- output side ----
\node[box, right=13mm of bandbox] (fusion)
  {\textbf{Attention-based mode fusion}\\[1pt]
   $w_k\propto\exp(\mathbf{q}^{\!\top}\mathbf{W}\mathbf{z}^{(k)}_{\mathrm{Cu}})$\\
   $\mathbf{z}_{\mathrm{fused}}=\sum_{k=1}^{K} w_k\,\mathbf{z}^{(k)}_{\mathrm{Cu}}$};
\node[box, right=11mm of fusion] (heads)
  {\textbf{Per-horizon heads}\\[1pt]
   $\hat r_{t+h}=\mathrm{MLP}_h(\mathbf{z}_{\mathrm{fused}})$\\
   $h\in\{1,5,10,22\}$ trading days};

\draw[arr] (inp) -- node[above, font=\scriptsize] {$\mathbb{R}^{N\times L}$} (vmd);
\draw[arr] (vmd.east) -- ++(6mm,0) |- node[pos=0.62, right=0.5mm, font=\scriptsize]
  {$\mathbf{u}^{(k)}_t$} (gat.west);
\draw[arr] (ext.east) -- ++(6mm,0) |- node[pos=0.62, left=0.5mm, font=\scriptsize]
  {$\mathbf{z}^{(k)}_{\mathrm{Cu}}$} (fusion.west);
\draw[arr] (fusion) -- node[above, font=\scriptsize] {$\mathbb{R}^{d_h}$} (heads);
\end{tikzpicture}}
\caption{VMD-MFGNN forward pass, corresponding block-for-block to Algorithm~\ref{alg:vmdmfgnn}. An $N=8$ variable window of length $L=60$ trading days is decomposed by leakage-safe daily-refit VMD into $K=5$ frequency-band modes per variable (Algorithm line 1). The dashed stack denotes the per-band branch instantiated once for each of the $K$ bands, with its own embedding pair $\mathbf{E}^{(1)}_k,\mathbf{E}^{(2)}_k$ and its own adjacency $\mathbf{A}_k$, row-wise sparsified to top-$k=4$ of $N=8$ before being passed to the band's GAT as an edge list (lines 3--5); band-$k$ mode values are projected to $d_h$ and passed through the GAT at each timestep with residual connections and layer normalization, then through a per-variable LSTM over the $L$ timesteps, and copper's representation is read from node $0$ (lines 6--11). The $K$ band representations are combined by the learned-query attention fusion of Section~\ref{sec:method} (lines 13--14) and mapped to the four forecast horizons by separate heads (lines 15--16). The separate-graph-per-band structure inside the dashed stack is the architectural choice this paper tests; the pooled-graph ablation of Section~\ref{sec:ablation} replaces it with one graph shared across bands.}
