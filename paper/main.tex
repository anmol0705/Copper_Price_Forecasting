...
\begin{abstract}
We build a leakage-safe, capacity-matched evaluation protocol for variational mode decomposition (VMD) combined with graph neural networks (GNNs) for copper futures return forecasting, and use it to test whether a proposed per-frequency-band graph architecture, VMD-MFGNN, offers real forecasting value. Across seven models (VMD-MFGNN, ARIMA, XGBoost, LSTM, Transformer, VMD-LSTM, SimpleMTGNN) and four horizons (1, 5, 10, 22 trading days) on a single chronological 2010--2025 train/validation/test split, none of the seven beats a trivial zero-forecast (random-walk) baseline on mean absolute error at any horizon, and that baseline also wins root-mean-squared error at two of four horizons. We then diagnose, rather than merely observe, why VMD-MFGNN's central mechanism---one learned graph per frequency band---shows no measurable advantage over a pooled single-graph alternative: direct inspection of trained checkpoints shows the per-band graph embeddings underwent isotropic norm collapse during training, RMS magnitude shrinking roughly 32-fold from initialization, with two independently trained models' embedding directions agreeing at cosine similarity above 0.99---training rescaled the embeddings without ever learning directional structure. We trace this to coupled weight decay interacting with a weak task gradient once a near-uniform adjacency already suffices for a model whose predictive signal is carried mainly by its temporal pathway. The contribution is therefore not a winning architecture but (a) a rigorous, disclosed evaluation protocol including the naive-baseline comparison this literature typically omits, and (b) a quantified, citable account of a graph-representation failure mode relevant to low-node-count financial GNNs generally.
\end{abstract}
...
\begin{table}[ht]
\centering
\caption{Forecasting performance comparison across four horizons, test split (2022--2025). Best result per column shown in \textbf{bold}. VMD-MFGNN is the HPO-tuned model (Section~\ref{sec:hpo}); the six baselines are untuned.}
\label{tab:main}
\resizebox{\textwidth}{!}{%
...
Transformer     & 0.0231 & 0.0493 & 0.0575 & 0.1053 & 0.0177 & 0.0357 & 0.0420 & 0.0870 & 51.9 & 53.0 & \textbf{52.7} & 44.7 \\
...
VMD-MFGNN & 0.0184 & \textbf{0.0398} & 0.0552 & 0.0785 & 0.0125 & 0.0282 & 0.0407 & 0.0608 & 52.5 & 50.8 & 47.7 & 47.1 \\
\midrule
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../results/figures/fig4_model_comparison.pdf}
...
\begin{table}[ht]
\centering
\caption{Ablation study results, all four horizons, test split. All variants trained at the untuned default configuration (Section~\ref{sec:impl}); this is a different, smaller VMD-MFGNN instance than the HPO-tuned model in Table~\ref{tab:main}. No significance column (single-split protocol; see Section~\ref{sec:limitations}). Best result per column shown in \textbf{bold}.}
\label{tab:ablation}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l cccc cccc cccc}
\toprule
...
(b) No-VMD, matched-dim (88{,}836) & 0.0279 & 0.0400 & 0.0569 & 0.0763 & 0.0231 & 0.0280 & 0.0429 & 0.0575 & 48.4 & \textbf{53.0} & 46.1 & 49.4 \\
(b) No-VMD, matched-params (394{,}116) & \textbf{0.0181} & 0.0399 & 0.0556 & 0.0816 & \textbf{0.0123} & 0.0281 & 0.0404 & 0.0645 & 48.4 & 51.5 & 44.2 & 44.7 \\
...
(c) Pooled graph, matched-params (397{,}586) & 0.0225 & 0.0419 & 0.0552 & 0.0762 & 0.0168 & 0.0291 & 0.0401 & \textbf{0.0568} & 50.9 & 52.9 & 49.8 & \textbf{55.3} \\
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../results/figures/fig5_ablation.pdf}
\caption{RMSE and Directional Accuracy across the six ablation variants, one line per horizon.}
\label{fig:ablation}
\end{figure}

\textbf{This table does not support the paper's central architectural claim, and we report that plainly, stated precisely rather than more strongly than the numbers allow.} The precise claim is: row (a), the per-band full model, never attains the \emph{best-of-six} value in any of the twelve RMSE/MAE/DA columns. This is a different and weaker claim than saying the full model loses every head-to-head comparison against either pooled variant specifically, and it does not --- the full model in fact wins several individual head-to-head columns against both pooled variants, just never enough to be the single best of all six variants in any column. Against \texttt{pooled\_graph\_matched\_params} (397{,}586 vs.\ 393{,}476 params, $+$1.05\%, the comparison most directly targeting RQ1 with capacity approximately controlled), the full model wins RMSE at $h=1$ and $h=5$ and MAE at $h=1$ and $h=5$; against \texttt{pooled\_graph\_matched\_dim} (84{,}618 params, $-$78.5\%), the full model wins RMSE at $h=1$ and $h=22$, MAE at $h=1$, and DA at $h=22$. What the full model does not do, against either pooled variant, at any horizon, is win directional accuracy: \texttt{pooled\_graph\_matched\_params} beats the full model on DA at \emph{all four} horizons, including $h=1$ (48.6\% vs.\ 50.9\%), and \texttt{pooled\_graph\_matched\_dim} beats it on DA at three of four horizons ($h=1,5,10$). Taken together: the per-band full model is competitive with, and sometimes better than, the pooled-graph variants on RMSE/MAE, particularly at short horizons, but it does not show a directional-accuracy advantage from per-band graph construction anywhere in this table, and it never wins outright against the full six-way comparison on any metric. This is the honest summary we take forward into Section~\ref{sec:discussion}: not that per-band graphs lose everything, but that they do not demonstrate the advantage the architecture was designed to produce.

The no-VMD ablation (row (b)) is similarly mixed rather than a clean win for decomposition: the matched-params variant, which omits VMD entirely and consumes raw prices with a single per-band-shaped graph, attains the best RMSE and MAE of \emph{all six variants} at $h=1$, and is the worst performer on RMSE of all six variants at $h=22$ (0.0816). On DA at $h=22$, however, the no-VMD matched-params variant (44.7\%) is \emph{not} the worst of the six --- \texttt{pooled\_graph\_matched\_dim} is lower still (44.6\%) --- so ``worst on RMSE and DA at $h=22$'' is not accurate for this variant; only the RMSE half of that claim holds. The matched-dim no-VMD variant is markedly worse than every other variant on RMSE/MAE at $h=1$ (a raw, undecomposed price series appears harder for a small-capacity model to fit directly), but recovers to be competitive, and even wins DA, at $h=5$. The correlation-graph variant (row (d)), which keeps VMD and the per-band architecture but replaces the learned adjacency with a fixed correlation-threshold graph, wins the best RMSE of all six variants at $h=22$ and is close to the full model everywhere else---so even the comparison isolating learned versus fixed adjacency does not show a clear advantage for end-to-end graph learning over the untuned full model at this configuration.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../results/figures/fig2_frequency_graphs.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{../results/figures/fig3_attention_weights.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{../results/figures/fig1_vmd_decomposition.pdf}
...
\textbf{RQ1 (per-band versus pooled graph).} Not supported, and now explained rather than merely observed. Section~\ref{sec:ablation-results}'s precise reading of Table~\ref{tab:ablation} is that the per-band full model never attains the best-of-six value in any column, and specifically never beats either pooled-graph variant on directional accuracy at any horizon, while trading RMSE/MAE wins back and forth with them at individual horizons. Given the adjacency-collapse diagnosis in Section~\ref{sec:results} --- every per-band adjacency matrix converged to $\approx 1/N$ uniform weights, with two independently trained models sharing the same collapsed, undifferentiated embedding direction --- this negative ablation result is not a surprising or mysterious finding about graph topology; it is close to the expected outcome once the per-band mechanism carries almost no differentiated information to begin with. We therefore answer RQ1 in the negative for a stronger, more specific reason than ``the ablation numbers came out this way'': the per-band graph mechanism, as trained here, did not learn the kind of structure that could produce a per-band advantage, so its absence in the ablation table is consistent with, not contradictory to, the mechanistic finding.
...
Section~\ref{sec:discussion} answers this paper's research questions in full against the real results; we do not re-derive those answers here, only summarize them. The naive-baseline result (RQ0) is the finding that should anchor how the rest is read: none of the seven fitted models, including VMD-MFGNN, beats a trivial zero-forecast baseline on MAE at any horizon. Against that backdrop, RQ1 and RQ2 (per-band versus pooled graphs, and versus no-VMD/fixed-correlation alternatives) are answered in the negative, and, unlike a typical negative ablation result, we can say why: the per-band graph-learning mechanism collapsed to near-uniform edge weights during training (isotropic embedding norm decay under coupled weight decay, Section~\ref{sec:results}), so its failure to show an accuracy advantage is the expected consequence of a mechanism that did not learn differentiated structure, not an unexplained architectural puzzle. RQ3 (does VMD-MFGNN differ significantly from standard baselines) is answered with a qualified yes that survives multiple-testing correction against the deep-learning baselines specifically, while remaining indistinguishable from ARIMA throughout --- a real but narrow result that RQ0 puts in context rather than contradicts.

This paper's contribution is therefore not the architecture it set out to propose. It is the evaluation protocol --- leakage-safe VMD, capacity-matched ablations, a naive baseline this literature typically omits, and a multiple-testing-corrected significance test --- applied honestly enough to surface, rather than paper over, both the naive-baseline result and the mechanism behind the negative ablation finding, together with a quantified, transferable diagnosis of embedding collapse that we believe is relevant to other learned-graph financial GNN architectures at similarly low node counts, MBTI-Net-style designs included. A study that tests a hypothesis rigorously, explains why it failed, and reports both plainly is a more useful contribution to this literature than one that reports only confirming evidence.

Future work includes: (1) mitigating the diagnosed embedding collapse directly (excluding graph embeddings from weight decay, normalizing embeddings to the unit sphere before the bilinear product, or adding an explicit sparsity/entropy regularizer on the adjacency) and re-running the per-band-versus-pooled ablation once the mechanism is confirmed to be learning non-degenerate structure, since the current negative result cannot distinguish ``per-band graphs do not help'' from ``per-band graphs were never actually trained to convergence on real structure''; (2) a walk-forward evaluation protocol with multiple folds, enabling paired significance testing on the ablations; (3) multi-seed replication; (4) symmetric hyperparameter optimization across all compared models, budget permitting; (5) extending the naive-baseline and economic-significance checks (Section~\ref{sec:econ}) to other base-metal commodities to test whether the difficulty of beating a trivial forecast is specific to copper's 2022--2025 regime or a more general property of this problem class; and (6), conditional on (1) succeeding, revisiting whether a per-band graph mechanism that has genuinely learned differentiated structure behaves differently in the ablation comparison than the collapsed mechanism reported here.
